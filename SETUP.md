# SETUP.md — Routely: Smart Link Router & Blog CMS

This is a single Next.js 14 app (App Router). The backend is implemented as
Next.js **Route Handlers** under `app/api/**` — a cleanly separated backend
layer (models, auth, DB connection all live in `lib/`), not mixed into page
components. This satisfies the brief's "Next.js API routes if cleanly
architecture-separated" option, and avoids running two separate servers.
...
- `ADMIN_EMAIL` / `ADMIN_PASSWORD` — the single admin account used to log
  into `/admin`. There's no user-registration flow by design — see
  `interview.md` for the reasoning.
...
- **Login always fails** — double check `ADMIN_EMAIL` / `ADMIN_PASSWORD` in
  `.env.local` match exactly what you're typing (case-sensitive on email
  comparison is normalized to lowercase, password is not).
